...
void input_ev(const char* filename) {
  ...
  for (int s = 0; s < NUM_STAGES; s++) {
	...
	size_t read = fread(Evs[s][f][t], sizeof(double), ARRAY_LENGTH, fp);
...
void add_next(int* pos_to_check, int *ci, int i) {
  if (i / 4 > 0) pos_to_check[(*ci)++] = i - 4;
  if (i / 4 < 3) pos_to_check[(*ci)++] = i + 4;
  if (i % 4 > 0) pos_to_check[(*ci)++] = i - 1;
  if (i % 4 < 3) pos_to_check[(*ci)++] = i + 1;
}
...
double calcEvFiltered(const board_t &board) {
  ...
  bool flag_dg = false;
  for (int i = 0; i < 16; i++) {
    if (board[i] >= 15) flag_dg = true;
  }

  board_t evboard;
  if (flag_dg) {
    int pos_to_check[512]; int ci = 0; int cj = 0;
    bool updated[16] = {false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false};
    for (int i = 0; i < 16; i++) {
      if (board[i] >= 15) {
	updated[i] = true; add_next(pos_to_check, &ci, i);
      }
    }
    while (ci > cj) {
      int ind = pos_to_check[cj++];
      if (updated[ind]) continue;
      if (ind / 4 > 0) {
	if (updated[ind - 4] && board[ind - 4] == board[ind] + 1) {
	  updated[ind] = true; add_next(pos_to_check, &ci, ind); continue;
	}
      }
      if (ind / 4 < 3) {
	if (updated[ind + 4] && board[ind + 4] == board[ind] + 1) {
      ...
      if (ind % 4 > 0) {
	if (updated[ind - 1] && board[ind - 1] == board[ind] + 1) {
	  updated[ind] = true; add_next(pos_to_check, &ci, ind); continue;
	}
      }
      if (ind % 4 < 3) {
    ...
    for (int i = 0; i < 16; i++) {
      if (updated[i]) {
	evboard[i] = max(board[i] - 1, 0);
      } else {
	evboard[i] = board[i];
      }
    }
  } else {
    for (int i = 0; i < 16; i++) evboard[i] = board[i];
  }

  double ev = 0;
  int stage = get_stage(evboard);
  board_t filtered_boards[NUM_SPLIT];

  // 各フィルターを適用 (mapping 方式)
  apply_filters_from_mapping(evboard, filtered_boards);
...
double expectimaxPlay(int depth, const board_t& board, unordered_map<unsigned long long, double>& ev_table)
{
...
// printf("after contains no\n");
  ...
  for (int index = 0; index < 16; index++) {
    ...
    count++;
    board_t putBoard; board_t nextBoard; double score;
    ...
    for (int d = 0; d < 4; d++) {
      ...
      if (score == -1) continue;
      double sc = expectimaxPlay(depth-1, nextBoard, ev_table) + score;
      // printf("depth %d index %d num 1 d %d ==> %d  (cur. %d)\n", depth-1, index, d, sc, maxScore);
      if (maxScore < sc) maxScore = sc; // ゲームオーバー盤面の処理
    }
    if (maxScore == -DBL_MAX) maxScore = 0;
    sumScore += maxScore * 9;
    ...
    if (maxScore == -DBL_MAX) maxScore = 0; // ゲームオーバー盤面の処理
    sumScore += maxScore;
  }
					     ...
					     unordered_map<unsigned long long, double>& ev_table)
{
  ...
  for (int i = 0; i < 4; i++) {   // 方向ごと
    ...
    nextEv[i] = expectimaxPlay(depth, nextBoards[i], ev_table) + scores[i];
    #ifdef DEBUG_PLAY
    printf("move = %d, ev = %d\n", i, nextEv[i]);
    #endif
  }
